...
if(!isset($strFiltro) || strlen($strFiltro) == 0) {
    echo json_encode('Nada Encontrado');
    die();
}
...
$arrRetorno =  array_filter($paises, function ($pais) use ($strFiltro) {
        $nome = strtolower($pais['nome']);
        $strFiltro = strtolower($strFiltro);

        if(strpos($nome, $strFiltro) !== false) {
            return true;
        } else {
            return false;
        }
...
//file_put_contents('teste.txt', $retorno . PHP_EOL, FILE_APPEND);
